...
sys.path.append(os.path.join(ROOT_DIR, 'pointSIFT_op'))
...
def pointSIFT_module(xyz, points, radius, out_channel, is_training, bn_decay, scope, bn=True, use_xyz=True):
...
def pointSIFT_module_original(xyz, points, radius, out_channel, is_training, bn_decay, scope='point_sift', bn=True, use_xyz=True, use_nchw=False):
...
def pointSIFT_res_module_original(xyz, points, radius, out_channel, is_training, bn_decay, scope='point_sift', bn=True, use_xyz=True, same_dim=False, merge='add'):
    data_format = 'NHWC'
    with tf.variable_scope(scope) as sc:
        # conv1
        _, new_points, idx, _ = pointSIFT_group(radius, xyz, points, use_xyz=use_xyz)
